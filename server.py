    ...
    with app.app_context():
        ...
        if formatted_quiz != "FAILED":
            ...
            if completed == True:
                ...
                db.session.commit()
                notify_user(receiving_email=email, name=name, link=url)
                ...
                if lecture_slides.filename != "":
                    if lecture_slides.filename.split(".")[1] in SUPPORTED_FORMATS:
                        ...
                        threading.Thread(target=generate_quiz, kwargs={"lecture_slides": lecture_slides.filename,
                                                                       ...
                                                                       "url": url_for('main_quiz', index=int(0), id=current_user.quizzes[len(current_user.quizzes) - 1].id, score=0)}).start()
                    else:
                    ...
                    threading.Thread(target=generate_quiz, kwargs={"lecture_slides": lecture_slides.filename,
                                                                   ...
                                                                   "email": current_user.email,
                                                                   "name": current_user.name,
                                                                   "url": url_for('main_quiz', index=int(0),
                                                                                  id=current_user.quizzes[
                                                                                      len(current_user.quizzes) - 1].id,
                                                                                  score=0)}).start()
